...
function Backpack() {
	...
	this.checkCanAdd = function(thing) {
		switch(thing.type) {
			case 1:
				if (this.empty_list["장비"].length == 0) return false;
				break;
			case 2:
				for (var i in backpack["소비"]) {
					if (backpack["소비"][i] != null && backpack["소비"][i].name == thing.name 
						&& (backpack["소비"][i].amount != this.consumable_limit)
					 	&& (backpack["소비"][i].amount + thing.amount <= this.consumable_limit 
						|| backpack["소비"][i].amount + thing.amount > this.consumable_limit && this.empty_list["소비"].length != 0)) {
						this.consumable_pos = i;
						return true;
					}
				}
				this.consumable_pos = -1;
				if (this.empty_list["소비"].length == 0) return false;
				break;
			case 3:
				for (var i in backpack["기타"]) {
					if (backpack["기타"][i] != null && backpack["기타"][i].name == thing.name 
						&& (backpack["기타"][i].amount != this.other_limit)
					 	&& (backpack["기타"][i].amount + thing.amount <= this.other_limit 
						|| backpack["기타"][i].amount + thing.amount > this.other_limit && this.empty_list["기타"].length != 0)) {
						this.other_pos = i;
						return true;
					}
				}
				this.other_pos = -1;
				if (this.empty_list["기타"].length == 0) return false;
				break;
			case 4:
				break;		
		}
	...
	this.add = function(thing) {
		switch(thing.type) {
			case 1:
				var pos = (this.empty_list["장비"].splice(0, 1))[0];
				backpack["장비"][pos] = new EquipmentItem(thing.name, thing.curr_res);
				break;
			case 2:
				if (this.consumable_pos != -1) {
					if (backpack["소비"][this.consumable_pos].amount + thing.amount <= this.consumable_limit) {
						backpack["소비"][this.consumable_pos].amount += thing.amount;
					} else {
						var amount =  this.consumable_limit - backpack["소비"][this.consumable_pos].amount;
						backpack["소비"][this.consumable_pos].amount = this.consumable_limit;

						var pos = (this.empty_list["소비"].splice(0, 1))[0];
						backpack["소비"][pos] = new ConsumableItem(thing.name, amount, thing.curr_res);
					}

				} else {
					var pos = (this.empty_list["소비"].splice(0, 1))[0];
					backpack["소비"][pos] = new ConsumableItem(thing.name, thing.amount, thing.curr_res);
				}
				break;
			case 3:
				if (this.other_pos != -1) {
					if (backpack["기타"][this.other_pos].amount + thing.amount <= this.other_limit) {
						backpack["기타"][this.other_pos].amount += thing.amount;
					} else {
						var amount =  this.other_limit - backpack["기타"][this.other_pos].amount;
						backpack["기타"][this.other_pos].amount = this.other_limit;

						var pos = (this.empty_list["기타"].splice(0, 1))[0];
						backpack["기타"][pos] = new OtherItem(thing.name, amount, thing.curr_res);
					}

				} else {
					var pos = (this.empty_list["기타"].splice(0, 1))[0];
					backpack["기타"][pos] = new OtherItem(thing.name, thing.amount, thing.curr_res);
				}
				break;
			case 4:
				break;		
	...
	this.drawMenu = function(ctx) {
		if (this.select_point.index != -1) {
			...
			// ctx.roundRect(this.select_point.x + 1, this.select_point.y + 1, 48, 78, 10, false, true);
 			...
 			switch(this.open) {
 				case "장비":
 					ctx.fillText("장비", this.select_point.x + 25, this.select_point.y + 18);
 					ctx.fillText("버리기", this.select_point.x + 25, this.select_point.y + 48);
 					ctx.fillText("등록", this.select_point.x + 25, this.select_point.y + 78);	
 					ctx.fillText("거래", this.select_point.x + 25, this.select_point.y + 108);
 					break;
 				case "소비":
 					ctx.fillText("사용", this.select_point.x + 25, this.select_point.y + 18);
 					ctx.fillText("버리기", this.select_point.x + 25, this.select_point.y + 48);
 					break;
 				case "기타":
 					ctx.fillText("정보", this.select_point.x + 25, this.select_point.y + 18);
 					ctx.fillText("버리기", this.select_point.x + 25, this.select_point.y + 48);	
 					ctx.fillText("등록", this.select_point.x + 25, this.select_point.y + 78);	
 					ctx.fillText("거래", this.select_point.x + 25, this.select_point.y + 108);
 					break;
	...
	this.head = [];
	this.head["장비"] = [2, 3, 5, 7, 9];
	this.head["소비"] = [1, 4, 5, 7, 9];
	this.head["기타"] = [1, 3, 6, 7, 9];
	this.head["特殊"] = [1, 3, 5, 8, 9];
	this.head["사용"] = [1, 3, 5, 7, 10];
	...
	this.selectResult = function() {
		...
		if (this.select_point.select == 1) {
			...
			switch(this.open) {
				...
				case "소비":
					var name = backpack[this.open][this.select_point.index].name;
					if (name == "빨간 포션" || name == "白色药水" || name == "橙色药水") {
						this.hp_set_index = this.select_point.index;
					} else if (name == "파란 포션" || name == "活力神水") {
	...
	this.addMp = function() {
		...
		backpack["소비"][this.mp_set_index].amount--;
		var name = backpack["소비"][this.mp_set_index].name;
		...
		if (backpack["소비"][this.mp_set_index].amount == 0) {
			var name = backpack["소비"][this.mp_set_index].name;
			for (var i in backpack["소비"]) {
				if (backpack["소비"][i] != null && i != this.mp_set_index && name == backpack["소비"][i].name) {
					this.mp_set_index = i;
					return;
				}
			}
			...
			backpack["소비"][this.mp_set_index] = null;
	...
	this.addHp = function() {
		...
		backpack["소비"][this.hp_set_index].amount--;
		var name = backpack["소비"][this.hp_set_index].name;
		var hp = 0;
		if (name == "빨간 포션") {
			hp = 50;
		} else if (name == "橙色药水") {
		...
		if (backpack["소비"][this.hp_set_index].amount == 0) {
			var name = backpack["소비"][this.hp_set_index].name;
			for (var i in backpack["소비"]) {
				if (backpack["소비"][i] != null && i != this.hp_set_index && name == backpack["소비"][i].name) {
					this.hp_set_index = i;
					return;
				}
			}

			backpack["소비"][this.hp_set_index] = null;
...
function DesFactory() {
	this.equipmentDes = function(properties) {
		...
		if (properties.attack != 0) {
			arr.push("공격력 " + properties.attack);
		}
		if (properties.defense != 0) {
			arr.push("물리방어력 " + properties.defense);
		}
		if (properties.magic_defense != 0) {	
			arr.push("마법방어력 " + properties.magic_defense);
		}
		return arr;
	}
